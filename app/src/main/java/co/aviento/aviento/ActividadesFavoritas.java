...
public class ActividadesFavoritas extends MapaActividades implements ClusterManager.OnClusterClickListener<MyItem>, ClusterManager.OnClusterInfoWindowClickListener<MyItem>, ClusterManager.OnClusterItemClickListener<MyItem>, ClusterManager.OnClusterItemInfoWindowClickListener<MyItem>
{
    ...
    private void readItems()
    {
        ...
        {
            GeoPunto p = favorito.getActividad().getEmpresa().getPosicion();
            if(p!= null && p.getLatitud() != 0)

                if (getActividades() == null  || favorito.getActividad().getTipo().getTexto().equals(getActividades()) || getActividades().equals("** Activity **") )
                {
                    Log.d(getActividades(), favorito.getActividad().getTipo().getTexto());
                    if (favorito.getActividad().getEmpresa().getPaquete()== 1)
                    {
                        mClusterManager.addItem(new MyItem(i, p.getLatitud(),  p.getLongitud(), favorito.getActividad().getNombre(), favorito.getActividad().getEmpresa().getDireccion(),favorito.getActividad().getEmpresa().getPaquete(),
                                favorito.getActividad().getTipo().getRecurso()));
                    }
                    else
                    {
                        mClusterManager.addItem(new MyItem(i, p.getLatitud(),  p.getLongitud(), favorito.getActividad().getNombre(), favorito.getActividad().getEmpresa().getDireccion(),favorito.getActividad().getEmpresa().getPaquete(),
                                favorito.getActividad().getTipo().getRecursoPremium()));
                    }
